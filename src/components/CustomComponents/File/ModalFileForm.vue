...
const setFormCreate = async () => {
  ...
  const { response, data } = await useApi(`file/create`).get();
...
const setFormEdit = async (id: string) => {
  handleClearForm();
  componentData.isLoading = true;
  const { response, data } = await useApi(`file/${id}/edit`).get();
  componentData.isLoading = false;
...
const handleSubmit = async () => {
  ...
  if (validation?.valid) {
    ...
    if (!componentData.form.id) {
      componentData.isLoading = true;
      responseApi = await useApi(`/file/store`).post(formData);
      componentData.isLoading = false;
    } else {
      componentData.isLoading = true;
      responseApi = await useApi(`/file/update/${componentData.form.id}`).post(formData);
      componentData.isLoading = false;
    }
