...
const startUpload = async () => {
  ...
  if (validation?.valid) {
    ...
    const uploadSingleFile = async (file: FileUpload) => {
      ...
      formData.append('support_type_id', support_type_id.value.value);
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          xhr.open('POST', `${api.defaults.baseURL}/file/uploadFiles`);
      ...
      <v-card-text>
        ...
        <div v-if="currentStep === 'upload'" class="upload-section">
          ...
          <VForm ref="refForm" @submit.prevent="() => { }">
            <div class="file-info mt-4">
              <AppSelectRemote :requiredField="true" :rules="[requiredValidator]" label="Tipo de soporte"
                v-model="support_type_id" url="/selectInfiniteSupportType" array-info="supportType" clearable
                :params="{ company_id: authenticationStore.company.id, module: 'SUPPORT_TYPE_MODULE_002' }">
              </AppSelectRemote>
            </div>
          </VForm>
