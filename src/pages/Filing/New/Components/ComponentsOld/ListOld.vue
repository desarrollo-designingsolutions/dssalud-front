...
const finishFiling = async () => {
  ...
  if (response.status == 200 && data) {
    ...
    if (countData > 0) {
      ...
      refModalQuestion.value.componentData.title = `Desea finalizar ${countData} facturas en estado preradicado?`
      refModalQuestion.value.componentData.subTitle = `Tenga en cuenta que tiene ${countDataWithOutSupports} factura sin soporte`
    }
  }
...
const goViewUsers = (item: any) => {
  router.push({ name: "Filing-New-ListUsersOld", params: { id: filing_id, invoice_id: item.id } })
}
