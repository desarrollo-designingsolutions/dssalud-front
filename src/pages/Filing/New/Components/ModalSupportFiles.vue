...
const openModal = async (filing_invoice_id: string) => {
  handleClearForm();
  handleDialogVisible();
  resetValues();

  form.value.filing_invoice_id = filing_invoice_id
};
...
const submitForm = async () => {
  ...
  formData.append('support_type_name', form.value.support_type_id.title);
      ...
      <VCard :loading="isLoading" class="w-100">
        ...
        <VCardText :disabled="isLoading">
          <VForm ref="formValidation" @submit.prevent="() => { }">
            <VRow>
              <VCol cols="12" md="6">
                <SelectInfiniteSupportType v-model="form.support_type_id" :rules="[requiredValidator]" />
              </VCol>
            </VRow>
          </VForm>
        </VCardText>
