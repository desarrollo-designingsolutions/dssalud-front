...
const titleModal = ref<string>("Errores en la validación de asignaciones")
...
const downloadExcel = async () => {
  loading.excel = true;
  const { data, response } = await useAxios("/assignment/excelErrorsValidation").get({
    params: {
...
const downloadCsv = async () => {
  loading.excel = true;
  const { data, response } = await useAxios("/assignment/exportCsvErrorsValidation").get({
    params: {
      user_id: user_id.value,
    }
  })
  ...
  if (response.status == 200 && data) {
    downloadExcelBase64(data.excel, "Asignaciones", ".csv")
  }
}
