...
const validationExcel = ref(null);
const validationXml = ref(null);

const init = async () => {
  errorMessages.value = []
  loading.getData = true
  const { data, response } = await useApi(`/rip/showErrorsValidation`).post({
    id: objData.value.id,
  })
  ...
  if (response.value?.ok && data.value) {
    ...
    validationExcel.value = data.value.validationExcel
    validationXml.value = data.value.validationXml
  }
}
...
const downloadExcel = async () => {
  loading.excel = true;
  const { data, response } = await useApi("/rip/excelErrorsValidation").post({
    id: objData.value.id,
  })
...
const submitConfirm = async () => {
  ...
  loading.getData = true
  const { data, response } = await useApi(`/rip/storeJson`).post({
    id: objData.value.id,
    ...
    <VDialog v-model="isDialogVisible" :overlay="false" transition="dialog-transition" persistent>
      ...
      <VCard :loading="loading.getData" :disabled="loading.getData" class="w-100">
        ...
        <VCardText class="d-flex justify-end gap-3 flex-wrap">
          ...
          <VBtn v-if="(validationTxt.totalSuccessfulInvoices > 0 && objData.numInvoices == 0)"
            :disabled="loading.getData" :loading="loading.getData" @click="openModalQuestion()" color="primary">
            Continuar
          </VBtn>
        </VCardText>
      </VCard>
