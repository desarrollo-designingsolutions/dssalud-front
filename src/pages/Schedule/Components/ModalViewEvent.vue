...
const emits = defineEmits(["modify", "close", "delete"]);
...
const getData = async (event_id: any) => {
  try {
    ...
    const { data, response } = await useAxios(`/schedule/getDataEvent/${event_id}`).get({
      params: {
  ...
  <VDialog v-model="isDialogVisible" max-width="1000" persistent class="user-details-dialog">
    ...
    <VCard :loading="isLoading" :disabled="isLoading">
              ...
              <VCardText class="pa-4">
                <VList class="transparent">
                  ...
                  <VListItem class="px-0 py-2">
                    ...
                    <VListItemSubtitle class="text-body-1 font-weight-medium text-wrap">
                      {{ eventData.user.length > 0 ? eventData.user[0].full_name : 'No asignado' }}
                    </VListItemSubtitle>
                  </VListItem>
          ...
          <VCol cols="12" lg="7">
            <VCard variant="outlined" class="team-card h-100" elevation="2">
              ...
              <VCardText class="pa-4 team-content">
                <div v-if="eventData.user.length > 0" class="team-scroll-container">
                  <div class="team-grid">
                    <VCard v-for="(el, index) in eventData.guests" :key="index" variant="outlined" class="team-member-card"
                      hover>
                      <VCardText class="pa-4">
      ...
      <VCardText class="d-flex justify-end gap-3 flex-wrap">
        <VSpacer />
        <VBtn color="error" :disabled="isLoading" @click="handleDelete" class="me-3">
          Eliminar
        </VBtn>
